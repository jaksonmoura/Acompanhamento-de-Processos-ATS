  ...
  <div class="filter">
    ...
    <?php foreach ($months as $month): ?>
        ...
        $year = date('Y');
        $result = $link->query("select count(*) as count from directories where month(created_at) = month('$date') and year(created_at) = '$year'");
        $qtdelt = mysqli_fetch_assoc($result);
        ?>
        <li><a href="list_all.php?date=<?php echo $date ?>"><?php echo $month.' ('.$qtdelt['count'].')'; ?></a></li>
    <?php endforeach ?>
    </ul>
    <form class="filter_form" action="list_all.php" method="get">
